...
[iostream 基本的输入输出](http://www.runoob.com/cplusplus/cpp-basic-input-output.html)
...
[指针的使用](http://www.runoob.com/cplusplus/cpp-pointers.html)  
...
[C++引用](http://www.runoob.com/cplusplus/cpp-references.html)
******  
...
C++ 接口是使用抽象类来实现的，抽象类与数据抽象互不混淆，数据抽象是一个把实现细节与相关的数据分离开的概念。如果类中至少有一个函数被声明为**纯虚函数**，则这个类就是**抽象类**。  
...
**原子操作的实现跟普通数据类型类似，但是它能够在保证结果正确的前提下，提供比mutex等锁机制更好的性能。开发过程中,对于多线程的情况下,单个基础数据类型的数据共享安全,尽量使用原子操作代替锁机制. 当需要对代码块进行数据安全保护的时候,就需要选择使用锁机制了.**

[C++ 原子操作（6种原子顺序）](https://blog.csdn.net/what951006/article/details/78273903)  
...
http get post
tcp socket  
[C++ 一个简单的实现HTTP GET和POST请求的类](https://blog.csdn.net/u014049599/article/details/79998674)
...
**多继承，如果多个父类有虚函数，那么该对象内存分布上，就存在多个虚函数表指针，每个虚表指针指向其对应的虚函数表。**
...
[c++11的右值引用&&](https://blog.csdn.net/u011583798/article/details/56488305)

一个左值表达式代表的是 **对象本身** ，而右值表达式代表的是**对象的值**；**变量也是左值**。

为了支持**移动操作（包括移动构造函数和移动赋值函数）**，C++才引入了一种新的引用类型——右值引用，可以**自由接管右值引用的对象内容**。
...
[new和malloc区别和malloc详解](https://blog.csdn.net/zhong29/article/details/80930919)
...
[C++构造函数和析构函数调用虚函数时都不会使用动态联编](https://www.cnblogs.com/bonelee/p/5826196.html)
...
[C++中如何在指定的内存中创建新对象](https://blog.csdn.net/gao1440156051/article/details/51442003)
...
[memmove 和 memcpy的区别](https://www.cnblogs.com/luoquan/p/5265273.html)
